...
from codes.gene_expression import make_umaps
...
def refine_processed_data(project_name, paths, project_cfg, global_cfg, samples_to_process):
    ...
    color_config_path = os.path.join("python_common_scripts", f"global_colors.json")
    
    if os.path.exists(color_config_path):
        data = json.load(open(color_config_path)); gene_color_map, cell_type_colors = data.get("gene_color_map", {}), data.get("cell_type_colors", {})
    else:
        gene_color_map, cell_type_colors = dict(GENE_COLOR_MAP), dict(CELL_TYPE_COLORS)
        ...
        try:
            ...
            if os.path.exists(adata_annotated_path):
                ...
                if os.path.exists(adata_qc_path):
                    ...
                    print(f"Missing QC file for {sample_id}, skipping"); continue

                adata_annotated, gene_color_map, cell_type_colors = cell_annotation_wrapper(sample_id, adata_qc, adata_annotated_path, markers_file_path, cell_annotation_file_path,gene_color_map, cell_type_colors, genes_list = important_genes, cell_type_label_col = "predicted_cell_type", palette="glasbey")

            

            

            adata_umap, gene_color_map, cell_type_colors = make_umaps(sample_id, adata_annotated, umap_individual_save_dir, gene_color_map, cell_type_colors, cols_to_plot, genes_list = important_genes, cell_type_label_col="predicted_cell_type", palette="glasbey", plot_always=True)
    ...
    json.dump({"gene_color_map": gene_color_map, "cell_type_colors": cell_type_colors}, open(color_config_path, "w"))